-- order by 
-- order by 절은 select 절과 함께 사용을 하며 결과 집합을 특정 열이나,
-- 열들의 값에 따라서 정렬하는데 사용된다.
...
order by	 	 	-- 정렬의 기준을 작성한다.
	menu_price desc; -- asc : 오름차순 정렬, desc : 내림차순 정렬
...
-- tbl_menu 테이블에서 메뉴이름과 판매상태를 조회하는데 판매 상태가 Y 인 친구들이 먼저 정렬이 되게 조회해줘
select
	menu_name,
    orderable_status
from
