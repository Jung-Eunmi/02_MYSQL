...
where
	menu_name = '민트미역국';
-- 2. 민트미역국의 카테고리 코드는 4, 카테고리 코드가 4인 메뉴 조회
select
	*
from
	tbl_menu
where
	category_code = 4;
-- 반칙을 쓰지 않는 이상 1개의 쿼리문으로 절대 할 수 없다.
...
-- max() : 최대값, min() : 최소값
-- from 절에 사용하는 서브쿼리는 (derived table : 파생테이블) 이라고 불린다.
...
-- 상관 서브쿼리란?
-- 메인쿼리문 서브쿼리의 결과에 영향을 주는 경우를 상관 서브쿼리라고 한다.
