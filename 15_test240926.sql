...
select
	substring(HIRE_DATE ,3,2) 입사년도,
    substring(HIRE_DATE ,6,2) 입사월,
    substring(HIRE_DATE ,9,2) 입사날짜
from
	EMPLOYEE;

-- 3. WHERE 절에서 함수 사용도 가능하다.
-- 여직원들의 모든 컬럼 정보를 조회
select
	*
from
...
-- 4. 함수 중첩 사용 가능 : 함수안에서 함수를 사용할 수 있음
...
-- 6. EMPLOYEE 테이블에서 사원의 이름, 입사일, 입사후 6개월이
-- 되는 날짜를 조회
select
	EMP_NAME,
    HIRE_DATE,
...
-- 7. EMPLOYEE 테이블에서 근무 년수가 20년 이상인 직원 조회
select
	*
from
	EMPLOYEE
where
	DATEDIFF(now(),HIRE_DATE) >= 7300;

-- 8. EMPLOYEE 테이블에서 사원명, 입사일, 
-- 입사한 월의 근무일수를 조회하세요
select
	EMP_NAME,
	HIRE_DATE,
